...
for i=1:14
    cc=i*2-1;
    ii=ii+1;
    Yasuo(ii)=min(Departure_1(cc+1,:)-Departure_1(cc,:))-120;
        if Yasuo(ii)<0
            disp('出错');
        else
            if Yasuo(ii)>0
                Departure_1(cc+1,:)=Departure_1(cc+1,:)-Yasuo(ii);
                Arrival_1(cc+1,:)=Arrival_1(cc+1,:)-Yasuo(ii);
            end
        end
    ii=ii+1;
    if i==1
    if VCor15(i,1)==0
        Yasuo(ii)=min(Departure_5(i,18:26)-Departure_1(cc+1,16:24))-30;
        if Yasuo(ii)<0
            disp('出错');
        else
            if Yasuo(ii)>0
        ...
        if min(Departure_5(i,18:26)-Departure_1(cc+1,16:24))-min(Departure5(i,18:26)-Departure1(cc+1,16:24))~=0
            kk(ii)=min(Departure_5(i,18:26)-Departure_1(cc+1,16:24))-min(Departure5(i,18:26)-Departure1(cc+1,16:24));
            Departure_1(cc+1,:)=Departure_1(cc+1,:)+kk(ii);
            Arrival_1(cc+1,:)=Arrival_1(cc+1,:)+kk(ii);
        end
    else
        if VCor15(i,1)==1
            Yasuo(ii)=min(Departure_5(i,18:26)-Departure_1(cc+1,16:24))-120;
        if Yasuo(ii)<0
            disp('出错');
        else
            if Yasuo(ii)>0
        ...
        if i>1
            if VCor15(i,1)==0
                 Yasuo(ii)=min([Departure_5(i,18:26)-Departure_1(cc+1,16:24)-30,Departure_5(i,:)-Departure_5(i-1,:)-120]);
                 if Yasuo(ii)<0
                     disp('出错');
                 else
                     if Yasuo(ii)>0
                 ...
                 if VCor15(i,1)==1
                     Yasuo(ii)=min([Departure_5(i,18:26)-Departure_1(cc+1,16:24)-120,Departure_5(i,:)-Departure_5(i-1,:)-120]);
                     if Yasuo(ii)<0
                         disp('出错');
                     else
                         if Yasuo(ii)>0
    ...
    ii=ii+1;
    if VCor15(i,2)==0
        Yasuo(ii)=min([Departure_1(cc+2,16:24)-Departure_5(i,18:26)-30,Departure_1(cc+2,:)-Departure_1(cc+1,:)-120]);
        if Yasuo(ii)<0
            disp('出错');
        else
            if Yasuo(ii)>0
                Departure_1(cc+2,:)=Departure_1(cc+2,:)-Yasuo(ii);
                Arrival_1(cc+2,:)=Arrival_1(cc+2,:)-Yasuo(ii);
            end
        end
        if min(Departure_1(cc+2,16:24)-Departure_5(i,18:26))-min(Departure1(cc+2,16:24)-Departure5(i,18:26))~=0
            kk(ii)=min(Departure_1(cc+2,16:24)-Departure_5(i,18:26))-min(Departure1(cc+2,16:24)-Departure5(i,18:26));
            Departure_5(i,:)=Departure_5(i,:)+kk(ii);
            Arrival_5(i,:)=Arrival_5(i,:)+kk(ii);
        end
    else
        if VCor15(i,2)==1
            Yasuo(ii)=min([Departure_1(cc+2,16:24)-Departure_5(i,18:26)-120,Departure_1(cc+2,:)-Departure_1(cc+1,:)-120]);
        if Yasuo(ii)<0
            disp('出错');
        else
            if Yasuo(ii)>0
...
for i=1:14
    ...
    for j=18:25
        ...
        y=[];
        if VCor15(i,1)==0
            x(1)=double(Departure_1(2*i,j-2))-60;
            y(1)=c;
            x(2)=double(Departure_5(i,j))+60;
            y(2)=c;
            x(3)=double(Arrival_5(i,j+1))+60;
            y(3)=c-10;
            x(4)=double(Arrival_1(2*i,j-1))-60;
            y(4)=c-10;
            x(5)=x(1);
            y(5)=y(1);
            fill(x,y,'c');
            ...
            x=[];
            y=[];
            x(1)=double(Departure_1(2*i+1,j-2))-60;
            y(1)=c;
            x(2)=double(Departure_1(2*i+1,j-2))+60;
            y(2)=c;
            x(3)=double(Arrival_1(2*i+1,j-1))+60;
            y(3)=c-10;
            x(4)=double(Arrival_1(2*i+1,j-1))-60;
            y(4)=c-10;
            x(5)=x(1);
            y(5)=y(1);
            fill(x,y,'c');
            ...
            if VCor15(i,2)==0
                ...
                x(1)=double(Departure_1(2*i,j-2))-60;
                y(1)=c;
                x(2)=double(Departure_1(2*i,j-2))+60;
                y(2)=c;
                x(3)=double(Arrival_1(2*i,j-1))+60;
                y(3)=c-10;
                x(4)=double(Arrival_1(2*i,j-1))-60;
                y(4)=c-10;
                x(5)=x(1);
...
UIC4061=(Departure_1(end-2,1)-Departure_1(1,1))/(Departure1(end-2,1)-Departure1(1,1));
UIC4065=(Departure_5(end-1,1)-Departure_5(1,1))/(Departure5(end-1,1)-Departure5(1,1));
UIC406z=(Departure_1(end-2,1)-Departure_1(1,1)+Departure_5(end-1,1)-Departure_5(1,1))/(Departure1(end-2,1)-Departure1(1,1)+Departure5(end-1,1)-Departure5(1,1));

deltaD1=Departure1-Departure_1;
deltaD5=Departure5-Departure_5;
for i=1:15
    deltaD(3*i-2,1)=deltaD1(2*i-1,1);
    deltaD(3*i-1,1)=deltaD1(2*i,1);
    deltaD(3*i,1)=deltaD5(i,1);
end
delta21=deltaD(2:end,1)-deltaD(1:end-1,1);
