...
for i=1:Totaltrain3-1
    a=160;
    for j=8:15
        if VCor35(i,1)==0
            x=[];
            y=[];
            x(1)=double(Departure3(i,j))-50;
            y(1)=a;
            x(2)=double(Departure3(i,j))+80;
            y(2)=a;
            x(3)=double(Arrival3(i,j+1))+80;
            y(3)=a-10;
            x(4)=double(Arrival3(i,j+1))-50;
            y(4)=a-10;
            x(5)=x(1);
            y(5)=y(1);
            A=fill(x,y,'y');
            ...
            y(1)=a-2;
            x(2)=double(Arrival3(i,j));
            y(2)=a+2;
            x(3)=double(Departure3(i,j));
            y(3)=a+2;
            x(4)=double(Departure3(i,j));
            y(4)=a-2;
            x(5)=x(1);
            y(5)=y(1);
            A=fill(x,y,'y');
        ...
        if VCor35(i,2)==0
            x=[];
            y=[];
            x(1)=double(Departure3(i,j))-80;
            y(1)=a;
            x(2)=double(Departure3(i,j))+50;
            y(2)=a;
            x(3)=double(Arrival3(i,j+1))+50;
            y(3)=a-10;
            x(4)=double(Arrival3(i,j+1))-80;
            y(4)=a-10;
            x(5)=x(1);
            y(5)=y(1);
            A=fill(x,y,'y');
...
for i=1:Totaltrain3-1
    ...
    for j=1:20
        ...
        y(2)=b;
        if double(Qinvehicle_line3_left(i,j))+double(Qboard_line3_left(i,j))<=TrainCapacity*0.5%满载率小于50%
            color=[0.13333,0.5451,0.13333];
        else if double(Qinvehicle_line3_left(i,j))+double(Qboard_line3_left(i,j))<=TrainCapacity*0.6%满载率小于60%
                color=[0.67843,1,0.18431];
            else if double(Qinvehicle_line3_left(i,j))+double(Qboard_line3_left(i,j))<=TrainCapacity*0.7%满载率小于70%
                    color=[1,0.84314,0];
                else if double(Qinvehicle_line3_left(i,j))+double(Qboard_line3_left(i,j))<=TrainCapacity*0.8%满载率小于80%
                        color=[0.95686,0.64314,0.37647];
                    else if double(Qinvehicle_line3_left(i,j))+double(Qboard_line3_left(i,j))<=TrainCapacity*0.9%满载率小于90%
                            color=[0.94118,0.50196,0.50196];
                        else if double(Qinvehicle_line3_left(i,j))+double(Qboard_line3_left(i,j))<=TrainCapacity*1%满载率小于100%
                                color=[0.69804,0.13333,0.13333];
                            else 
                                color=[1,0,0];
                            end
        ...
        x(1)=double(Departure3(i,j));
        y(1)=b;
        x(2)=double(Departure3(i,j))+100;
        y(2)=b;
        x(3)=double(Departure3(i,j))+100;
        y(3)=b-double(Qstranded_line3_left(i,j))/500;
        x(4)=double(Departure3(i,j));
        y(4)=b-double(Qstranded_line3_left(i,j))/500;
        x(5)=x(1);
        y(5)=y(1);
        if double(Qstranded_line3_left(i,j))<=300
            fill(x,y,'g');
            hold on
        else if double(Qstranded_line3_left(i,j))<=500
                fill(x,y,'c');
                hold on
            else if double(Qstranded_line3_left(i,j))<=1000
                    fill(x,y,'y');
                    hold on
                else if double(Qstranded_line3_left(i,j))<=2000
                        fill(x,y,'b');
                        hold on
                     else if double(Qstranded_line3_left(i,j))<=4000
                             fill(x,y,'m');
                             hold on
                         else 
                             fill(x,y,'r');
...
ah=axes('position',get(gca,'position'),...
    'visible','off');
legend(ah,St,'(0,100]','(100,300]','(300,500]','(500,700]','(700,1000]','>1000','Location','NorthEastOutside');
lgd2=legend;
